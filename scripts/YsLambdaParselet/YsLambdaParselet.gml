...
function YsLambdaParselet(precedence) : GsplInfixParselet(precedence) constructor {
	...
	static parse = function(parser, left_expr, token) {
		...
		// track the previous lambda context if one existed and set up a new context
		// (lambdas can be nested)
		var old_context = parser.lambda_context;
		parser.lambda_context = new YuiLambdaContext(old_context);
		
		// set up the args -> param name lookup
		if is_instanceof(left_expr, YuiIdentifier) {
			var param_name = left_expr.resolve();
			parser.lambda_context.arg_map = [param_name];
			...
			parser.lambda_context.arg_map = array_create(param_count);
			...
			var i = 0; repeat param_count {
				...
				parser.lambda_context.arg_map[i] = param_name;
				i++;
			}
		}
		else {
		...
		var lambda = new YuiLambda(body, parser.lambda_context);
		
		// reset the lambda context to whatever it was before
		parser.lambda_context = old_context;
