...
function yui_world_to_gui_y_2(y, camera = 0) {
	if view_enabled {
		...
		// multiple by the viewport size to get the y position in the view
		var y_in_viewport = camera_ratio_y * viewport_h;
		return floor(y_in_viewport);
	}
	else {
		...
		// convert to GUI scale
		var gui_y = room_ratio_y * display_get_gui_height();

		return floor(gui_y);
	}
}

/// @desc convert room y to window y
/// @param {Real} y
/// @param {Id.Camera,Real} camera
function yui_world_to_window_y(y, camera = 0) {
	if view_enabled {
		var camera_y = camera_get_view_y(view_camera[camera]);
		var camera_h = camera_get_view_height(view_camera[camera]);
		var viewport_h = view_hport[camera];
