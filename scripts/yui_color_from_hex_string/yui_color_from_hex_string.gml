...
function yui_color_from_hex_string(hex_color) {
	...
	var color = make_color_rgb(yui_hex_to_dec(red), yui_hex_to_dec(green), yui_hex_to_dec(blue));
	
	// set alpha to 1 if we're only defining the color
	if string_length(hex_color) == 7 {
		color |= $FF000000;
	}
